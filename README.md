...
- [12. 订单主题 ETL 串并联执行蓝图（任务图 + 参数建议 + 故障回滚策略）](#12-订单主题-etl-串并联执行蓝图任务图--参数建议--故障回滚策略)
- [13. 总结](#13-总结)
...
这一节把“分层是目标、回流/分流是现实”落成可执行规范。核心原则：**不是数据库自动判断语义，而是工程规则 + 依赖 DAG + 批次治理来判断与约束**。
...
- 例外：必须反馈业务事实时，走独立反馈层（如 FWD），禁止污染原子明细层。
